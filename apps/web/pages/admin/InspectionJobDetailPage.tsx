...
const InspectionJobDetailPage: React.FC = () => {
  ...
  if (!job) return <div className="rounded-lg border border-ink-200 bg-white p-6 text-sm text-ink-500">Inspection job not found.</div>;

  return <div className="mx-auto max-w-5xl space-y-5">
    <header className="rounded-2xl bg-ink-950 p-6 text-white shadow-lg"><p className="font-mono text-[11px] uppercase tracking-[0.2em] text-accent-400">Named workflow commands</p><div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="font-serif text-3xl font-bold">{property?.address || 'Inspection job'}</h1><p className="mt-2 text-sm text-ink-400">{job.reportType}</p></div><span className="rounded-full border border-white/15 px-3 py-1.5 text-xs font-bold capitalize text-ink-300">{statusLabel}</span></div></header>
    ...
    <section className="rounded-2xl border border-ink-200 bg-white p-5 shadow-sm"><dl className="grid gap-4 md:grid-cols-2"><div><dt className="text-xs font-bold uppercase tracking-wider text-ink-500">Scheduled</dt><dd className="mt-1 font-semibold">{job.scheduledAt ? new Date(job.scheduledAt).toLocaleString() : 'Not scheduled'}</dd></div><div><dt className="text-xs font-bold uppercase tracking-wider text-ink-500">Inspector</dt><dd className="mt-1 font-semibold">{job.assignedInspectorId || 'Unassigned'}</dd></div><div><dt className="text-xs font-bold uppercase tracking-wider text-ink-500">Reviewer</dt><dd className="mt-1 font-semibold">{job.assignedReviewerId || 'Unassigned'}</dd></div><div><dt className="text-xs font-bold uppercase tracking-wider text-ink-500">Report</dt><dd className="mt-1 font-semibold">{job.reportId ? <Link className="text-accent-700 underline" to={`/app/admin/reports/${job.reportId}/edit`}>Open report workspace</Link> : 'No report linked'}</dd></div></dl>{job.notes ? <p className="mt-4 border-t border-ink-100 pt-4 text-sm text-ink-600">{job.notes}</p> : null}</section>

    <section className="rounded-2xl border border-ink-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><PlayCircle className="text-emerald-700" /><h2 className="font-bold text-ink-950">Workflow action</h2></div><p className="mt-1 text-sm text-ink-500">Only commands valid for the current job status are accepted by the server. Evidence and component quality gates run before field submission.</p><div className="mt-4 flex flex-wrap gap-2">{availablePrimary ? <button type="button" disabled={busy} onClick={() => void command(availablePrimary.command, availablePrimary.command === 'assign' ? { assignedInspectorId: inspectorId || job.assignedInspectorId } : {})} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-emerald-700 px-4 text-sm font-black text-white disabled:opacity-40">{busy ? <LoaderCircle size={16} className="animate-spin" /> : <PlayCircle size={16} />}{availablePrimary.label}</button> : null}{job.status === 'on_hold' ? <><select value={resumeStatus} onChange={(event) => setResumeStatus(event.target.value as InspectionJobStatus)} className="rounded-lg border border-ink-300 px-3 text-sm"><option value="assigned">Assigned</option><option value="inspection_started">Inspection started</option><option value="photos_uploading">Photos uploading</option><option value="photos_uploaded">Photos uploaded</option><option value="inspection_submitted">Fieldwork submitted</option></select><button type="button" disabled={busy} onClick={() => void command('resume', { resumeStatus })} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-emerald-700 px-4 text-sm font-black text-white"><PlayCircle size={16} /> Resume</button></> : null}</div></section>

    <div className="grid gap-5 lg:grid-cols-2">
      <section className="rounded-2xl border border-ink-200 bg-white p-5"><div className="flex items-center gap-2"><UserRoundCog className="text-accent-700" /><h2 className="font-bold text-ink-950">Assignments</h2></div><div className="mt-4 grid gap-3"><label className="text-xs font-bold uppercase tracking-wider text-ink-500">Inspector ID<input value={inspectorId} onChange={(event) => setInspectorId(event.target.value)} className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2 text-sm font-normal normal-case tracking-normal" /></label><label className="text-xs font-bold uppercase tracking-wider text-ink-500">Reviewer ID<input value={reviewerId} onChange={(event) => setReviewerId(event.target.value)} className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2 text-sm font-normal normal-case tracking-normal" /></label><button type="button" disabled={busy || (!inspectorId.trim() && !reviewerId.trim())} onClick={() => void command('reassign', { ...(inspectorId.trim() ? { assignedInspectorId: inspectorId.trim() } : {}), ...(reviewerId.trim() ? { assignedReviewerId: reviewerId.trim() } : {}), reason: 'Assignment updated from job workspace.' })} className="min-h-10 rounded-lg border border-ink-950 text-sm font-bold">Update assignments</button></div></section>
      <section className="rounded-2xl border border-ink-200 bg-white p-5"><div className="flex items-center gap-2"><CalendarClock className="text-accent-700" /><h2 className="font-bold text-ink-950">Schedule</h2></div><label className="mt-4 block text-xs font-bold uppercase tracking-wider text-ink-500">Date and time<input type="datetime-local" value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2 text-sm font-normal normal-case tracking-normal" /></label><button type="button" disabled={busy || !scheduledAt} onClick={() => void command('reschedule', { scheduledAt: new Date(scheduledAt).toISOString(), reason: 'Schedule updated from job workspace.' })} className="mt-3 min-h-10 w-full rounded-lg border border-ink-950 text-sm font-bold">Reschedule</button></section>
    </div>
