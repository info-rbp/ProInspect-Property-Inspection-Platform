...
export const StructureEditor: React.FC<{
  ...
  return <section className="rounded-2xl border border-stone-200 bg-white p-4" aria-labelledby="structure-heading">
    <div className="flex items-center gap-2"><FolderPlus size={18} className="text-amber-700" /><h2 id="structure-heading" className="font-bold text-stone-950">Report structure</h2></div>
    <p className="mt-1 text-xs leading-5 text-stone-500">Published templates remain immutable. These controls record report-specific additions and ordering.</p>
    ...
    <form onSubmit={submitArea} className="mt-4 flex gap-2">
      ...
      <input id="new-area-name" value={areaName} onChange={(event) => setAreaName(event.target.value)} placeholder="Add an unusual area" className="min-w-0 flex-1 rounded-lg border border-stone-300 px-3 py-2 text-sm" disabled={!cloudEnabled || busy} />
      <button type="submit" disabled={!cloudEnabled || busy || !areaName.trim()} className="grid h-10 w-10 place-items-center rounded-lg bg-stone-950 text-white disabled:opacity-30" aria-label="Add area"><Plus size={17} /></button>
    </form>

    {selectedArea ? <div className="mt-4 space-y-3 border-t border-stone-200 pt-4">
      <div className="flex items-end gap-2">
        <label className="min-w-0 flex-1 text-xs font-bold uppercase tracking-wider text-stone-500">Selected area<input value={renamedArea} onChange={(event) => setRenamedArea(event.target.value)} className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm font-medium normal-case tracking-normal text-stone-900" disabled={!cloudEnabled || busy} /></label>
        <button type="button" disabled={!cloudEnabled || busy || !renamedArea.trim() || renamedArea.trim() === selectedArea.name} onClick={() => void onRenameArea(selectedArea, renamedArea.trim())} className="grid h-10 w-10 place-items-center rounded-lg border border-stone-300 text-stone-700 disabled:opacity-30" aria-label="Rename selected area"><Pencil size={15} /></button>
      </div>
      <div className="flex gap-2">
        <button type="button" disabled={!cloudEnabled || busy || areaIndex <= 0} onClick={() => void onMoveArea(selectedArea.id, -1)} className="inline-flex min-h-9 flex-1 items-center justify-center gap-2 rounded-lg border border-stone-300 text-xs font-bold disabled:opacity-30"><ArrowUp size={14} /> Earlier</button>
        <button type="button" disabled={!cloudEnabled || busy || areaIndex < 0 || areaIndex === areas.length - 1} onClick={() => void onMoveArea(selectedArea.id, 1)} className="inline-flex min-h-9 flex-1 items-center justify-center gap-2 rounded-lg border border-stone-300 text-xs font-bold disabled:opacity-30"><ArrowDown size={14} /> Later</button>
      ...
      <form onSubmit={submitComponent} className="flex gap-2">
        ...
        <input id="new-component-name" value={componentName} onChange={(event) => setComponentName(event.target.value)} placeholder={`Add component to ${selectedArea.name}`} className="min-w-0 flex-1 rounded-lg border border-stone-300 px-3 py-2 text-sm" disabled={!cloudEnabled || busy} />
        <button type="submit" disabled={!cloudEnabled || busy || !componentName.trim()} className="grid h-10 w-10 place-items-center rounded-lg bg-amber-400 text-stone-950 disabled:opacity-30" aria-label="Add component"><Plus size={17} /></button>
      </form>

      {selectedComponent ? <div className="rounded-lg bg-stone-50 p-3">
        <p className="truncate text-xs font-bold text-stone-800">{selectedComponent.component}</p>
        <div className="mt-2 flex gap-2">
          <button type="button" disabled={!cloudEnabled || busy || componentIndex <= 0} onClick={() => void onMoveComponent(selectedArea.id, selectedComponent.id, -1)} className="inline-flex min-h-9 flex-1 items-center justify-center gap-2 rounded-lg border border-stone-300 bg-white text-xs font-bold disabled:opacity-30"><ArrowUp size={14} /> Earlier</button>
          <button type="button" disabled={!cloudEnabled || busy || componentIndex < 0 || componentIndex === selectedArea.components.length - 1} onClick={() => void onMoveComponent(selectedArea.id, selectedComponent.id, 1)} className="inline-flex min-h-9 flex-1 items-center justify-center gap-2 rounded-lg border border-stone-300 bg-white text-xs font-bold disabled:opacity-30"><ArrowDown size={14} /> Later</button>
