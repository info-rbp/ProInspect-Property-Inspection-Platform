...
export const ReportWorkspacePage: React.FC = () => {
  ...
  const { state, dispatch, saveComponent } = useReportWorkspace(agencyId, reportId);
  ...
  return (
    <div className="mx-auto max-w-[1600px] pb-16 text-stone-900">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)_390px]">
        <aside className="rounded-2xl border border-stone-200 bg-stone-100/70 p-3 xl:sticky xl:top-20 xl:h-[calc(100vh-7rem)] xl:overflow-auto"><div className="px-2 pb-3"><p className="font-mono text-[11px] uppercase tracking-[0.2em] text-stone-500">Inspection map</p><p className="mt-1 text-sm font-bold text-stone-900">{aggregate.areas.length} configured areas</p></div><AreaNavigator areas={aggregate.areas} selectedAreaId={state.selectedAreaId} onSelect={selectArea} /></aside>
        <main className="min-w-0 space-y-5"><AreaEditor area={area} selectedComponentId={state.selectedComponentId} onSelect={selectComponent} /><EvidenceLinker component={component} /></main>
