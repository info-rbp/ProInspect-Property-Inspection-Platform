...
export const ReportWorkspacePage: React.FC = () => {
  ...
  if (state.loading) return <div className="grid min-h-[55vh] place-items-center"><div className="text-center"><RefreshCcw className="mx-auto animate-spin text-accent-600" /><p className="mt-3 text-sm font-semibold text-ink-600">Preparing report workspace…</p></div></div>;
  ...
  return (
    <div className="mx-auto max-w-[1600px] pb-16 text-ink-900">
      <header className="relative overflow-hidden rounded-2xl bg-ink-950 px-5 py-6 text-white shadow-xl sm:px-7">
        <div className="absolute inset-y-0 right-0 w-1/3 opacity-20" aria-hidden="true" style={{ backgroundImage: 'repeating-linear-gradient(135deg, transparent 0 16px, #208992 16px 17px)' }} />
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><Link to={`/app/admin/reports/${reportId}`} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-ink-400 hover:text-white"><ArrowLeft size={14} /> Report record</Link><p className="mt-5 font-mono text-[11px] uppercase tracking-[0.25em] text-accent-400">{aggregate.report.inspectionType ?? 'inspection'} / revision {aggregate.report.workspaceRevision ?? 1}</p><h1 className="mt-1 max-w-4xl font-serif text-3xl font-bold leading-tight sm:text-4xl">{aggregate.report.propertyAddress}</h1><p className="mt-2 text-sm text-ink-400">{aggregate.report.reportType} · {aggregate.report.inspectionDate || 'Date pending'}</p></div><div className="flex flex-wrap items-center gap-2"><span role="status" aria-live="polite" className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold ${save.className}`}>{state.saveState === 'saved_locally' ? <CloudOff size={14} /> : <Cloud size={14} />}{save.label}</span><span className="rounded-full border border-white/15 px-3 py-1.5 text-xs font-bold capitalize text-ink-300">{aggregate.report.lifecycleStatus.replaceAll('_', ' ')}</span></div></div>
      </header>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)_390px]">
        <aside className="space-y-4 xl:sticky xl:top-20 xl:h-[calc(100vh-7rem)] xl:overflow-auto">
          <div className="rounded-2xl border border-ink-200 bg-ink-100/70 p-3"><div className="px-2 pb-3"><p className="font-mono text-[11px] uppercase tracking-[0.2em] text-ink-500">Inspection map</p><p className="mt-1 text-sm font-bold text-ink-900">{aggregate.areas.length} configured areas</p></div><AreaNavigator areas={aggregate.areas} selectedAreaId={state.selectedAreaId} onSelect={selectArea} /></div>
          <StructureEditor
