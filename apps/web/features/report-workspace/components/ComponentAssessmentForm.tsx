...
const fieldClass = 'mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2.5 text-sm text-stone-950 outline-none transition focus:border-amber-600 focus:ring-2 focus:ring-amber-200';
const labelClass = 'text-xs font-bold uppercase tracking-[0.12em] text-stone-500';
...
export const ComponentAssessmentForm: React.FC<{ component?: Component; onChange: (patch: Partial<Component>) => void }> = ({ component, onChange }) => {
  if (!component) return <aside className="rounded-2xl border border-dashed border-stone-300 bg-white p-8 text-sm text-stone-500">Choose a component to record the assessment.</aside>;
  return (
    <aside className="rounded-2xl border border-stone-200 bg-[#fffdf8] p-5 shadow-sm" aria-labelledby="component-heading">
      <div className="border-b border-stone-200 pb-4"><p className="font-mono text-[11px] uppercase tracking-[0.2em] text-amber-700">Structured record</p><h2 id="component-heading" className="mt-1 font-serif text-2xl font-bold text-stone-950">{component.component}</h2></div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <label className={`${labelClass} flex items-center gap-3 self-end rounded-lg border border-stone-200 bg-white px-3 py-3`}><input type="checkbox" checked={component.maintenanceRequired} onChange={(event) => onChange({ maintenanceRequired: event.target.checked })} className="h-4 w-4 accent-amber-600" /> Maintenance candidate</label>
      </div>
