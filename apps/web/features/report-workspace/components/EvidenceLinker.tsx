  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-5" aria-labelledby="evidence-heading">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div className="flex items-start gap-3"><span className="rounded-lg bg-stone-950 p-2 text-amber-400"><Link2 size={19} /></span><div><p className="font-mono text-[11px] uppercase tracking-[0.2em] text-amber-700">Evidence links</p><h2 id="evidence-heading" className="mt-1 text-base font-bold text-stone-950">Component evidence</h2><p className="mt-1 text-xs leading-5 text-stone-500">Link only evidence that supports this component. Originals remain immutable.</p></div></div>
        <div>
          ...
          <button type="button" disabled={!component || !context || uploading} onClick={() => inputRef.current?.click()} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-stone-950 px-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">
      ...
      {!references.length ? (
        <div className="mt-4 rounded-xl border border-dashed border-stone-300 bg-stone-50 p-5 text-center"><Camera className="mx-auto text-stone-400" size={22} /><p className="mt-2 text-sm font-semibold text-stone-700">No explicit evidence linked</p><p className="mt-1 text-xs leading-5 text-stone-500">Add context, defect or testing evidence before submitting this component.</p></div>
      ) : (
        <ol className="mt-4 space-y-3">
          {references.map((reference, index) => {
            const isVideo = reference.contentType?.startsWith('video/');
            return <li key={reference.photoId} className="rounded-xl border border-stone-200 bg-stone-50 p-3">
              <div className="flex items-start gap-3">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-white text-stone-600 shadow-sm">{isVideo ? <FileVideo2 size={19} /> : <Camera size={19} />}</span>
                <div className="min-w-0 flex-1">
                  <label className="block text-xs font-bold uppercase tracking-wider text-stone-500">Caption<input value={reference.caption ?? ''} onChange={(event) => patch(reference.photoId, { caption: event.target.value })} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium normal-case tracking-normal text-stone-900" placeholder="Describe what this evidence shows" /></label>
                  <label className="mt-3 block text-xs font-bold uppercase tracking-wider text-stone-500">Purpose<select value={reference.purpose ?? 'context'} onChange={(event) => patch(reference.photoId, { purpose: event.target.value as EvidencePurpose })} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium normal-case tracking-normal text-stone-900">{purposes.map((purpose) => <option key={purpose.value} value={purpose.value}>{purpose.label}</option>)}</select></label>
                  <p className="mt-2 truncate font-mono text-[10px] text-stone-500" title={reference.photoId}>{reference.photoId}</p>
                </div>
                <div className="grid gap-1">
                  <button type="button" disabled={index === 0} onClick={() => move(reference.photoId, -1)} className="grid h-9 w-9 place-items-center rounded-lg border border-stone-300 bg-white text-stone-700 disabled:opacity-30" aria-label={`Move ${reference.caption || reference.photoId} earlier`}><ArrowUp size={15} /></button>
                  <button type="button" disabled={index === references.length - 1} onClick={() => move(reference.photoId, 1)} className="grid h-9 w-9 place-items-center rounded-lg border border-stone-300 bg-white text-stone-700 disabled:opacity-30" aria-label={`Move ${reference.caption || reference.photoId} later`}><ArrowDown size={15} /></button>
