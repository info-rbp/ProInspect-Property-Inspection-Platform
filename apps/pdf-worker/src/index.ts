...
export function renderReportHtml(input: RenderInput): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><style>@page{size:A4;margin:15mm}@page{@bottom-right{content:"Page " counter(page) " of " counter(pages);font-size:8pt;color:#78716c}}body{font-family:Arial,sans-serif;color:#1c1917;font-size:9.5pt}header{border-bottom:3px solid #d97706;padding-bottom:12px;margin-bottom:20px}h1{font-size:24pt;margin:0}h2{font-size:15pt;border-bottom:1px solid #d6d3d1;padding-bottom:5px;margin-top:22px;break-after:avoid}p{line-height:1.55;white-space:pre-wrap}.identity{font:9pt monospace;color:#57534e}table{width:100%;border-collapse:collapse;font-size:8.5pt}th,td{border:1px solid #d6d3d1;padding:6px;vertical-align:top;text-align:left}th{background:#f5f5f4}tr,figure{break-inside:avoid}.gallery{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.gallery img{display:block;width:100%;height:75mm;object-fit:contain;background:#f5f5f4}.gallery figcaption{font:7pt monospace;color:#57534e;margin-top:3px}footer{margin-top:28px;border-top:1px solid #d6d3d1;padding-top:10px;color:#57534e;font-size:8pt}</style></head><body><header><div class="identity">${escapeHtml(input.templateId)} v${input.templateVersion} · report version ${escapeHtml(input.reportVersionId)}</div><h1>${address}</h1></header>${areaHtml}${gallery ? `<section><h2>Inspection evidence</h2><div class="gallery">${gallery}</div></section>` : ''}<footer>${limitation}</footer></body></html>`;
}
